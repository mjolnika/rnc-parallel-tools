# download stagger from https://www.ling.su.se/english/nlp/tools/stagger and swedish.bin
...
tag_dict = {'LE':'NONFLEX', '':'', 'ind/def': 'ind/def', 'sub': 'sub', 'obj': 'obj', 'def': 'def', 'nom': 'nom', 'gen': 'gen', 'inf': 'inf', 'sub/obj':'sub/obj', 'sup':'sup', 'kon':'subj', 'an':'compos', 'prf':'prf','pos':'pos', 'HS': 'APRO,poss,rel', 'PS': 'APRO,poss', 'VB': 'V', 'IE': 'PART,inf', 'PP': 'PR', 'PN': 'SPRO', 'NN': 'S', 'DT': 'ART', 'KN': 'CONJ', 'RG': 'NUM', 'SN': 'CONJ,subord', 'HD': 'ART,rel', 'JJ': 'A', 'HP': 'SPRO,rel', 'AB': 'ADV', 'PM': 'S,persn', 'RO': 'ANUM', 'IN': 'INTJ', 'HA': 'ADV,rel', 'PC': 'V,partcp', 'PL': 'PART,verb', 'UO': 'NONLEX', 'suv': 'supr', 'plu': 'pl', 'sin': 'sg', 'sin/plu': 'sg/pl', 'neu': 'n', 'utr': 'm-f', 'utr/neu': 'm-f/n', 'ind': 'indef', 'prs': 'praes', 'prt': 'praet', 'imp': 'imper', 'kom': 'comp', 'mas': 'm', 'sfo': 'med', 'sms': 'abbr', 'akt': 'act'}
...
def build_ana(token, lemma, pos1, gramm, curAddr, prevAddr):
    ...
    if bWord:
        try:
            ...
            print('not in dict:', pos1)
        ana += '<w><ana lex="' + escape(lemma) + '" gr="' + \
              pos1
        if len(gramm) > 0 and gramm != '_':
            gramm = gramm.lower().replace('-', '').split('|')
            for num, g in enumerate(gramm):
                gramm[num] = tag_dict[g]
                    #print(g)
            gramm = re.sub(',+', ',', ','.join(gramm))
            ana += ',' + gramm
        ana += '" />'
    ana += token
    if bWord:
